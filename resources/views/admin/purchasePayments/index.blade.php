...
@can('purchase_payment_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.purchase-payments.create') }}">
                        ...
                        <tr data-entry-id="{{ $purchasePayment->id }}">
                            ...
                            <td>
                                @can('purchase_payment_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.purchase-payments.show', $purchasePayment->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('purchase_payment_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.purchase-payments.edit', $purchasePayment->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('purchase_payment_delete')
                                    <form action="{{ route('admin.purchase-payments.destroy', $purchasePayment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('purchase_payment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.purchase-payments.massDestroy') }}",
