    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-notes.update", [$crmNote->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id">
                    @foreach($customers as $id => $customer)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $crmNote->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $customer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('added_by') ? 'is-invalid' : '' }}" type="number" name="added_by" id="added_by" value="{{ old('added_by', $crmNote->added_by) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\CrmNote::IS_CLIENT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_client') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_client_{{ $key }}" name="is_client" value="{{ $key }}" {{ old('is_client', $crmNote->is_client) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $crmNote->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
