    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transfers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('to_account') ? 'is-invalid' : '' }}" type="number" name="to_account" id="to_account" value="{{ old('to_account', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('from_account') ? 'is-invalid' : '' }}" type="number" name="from_account" id="from_account" value="{{ old('from_account', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Transfer::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'cleared') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
