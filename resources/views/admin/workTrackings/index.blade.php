...
@can('work_tracking_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.work-trackings.create') }}">
                        ...
                        <tr data-entry-id="{{ $workTracking->id }}">
                            ...
                            <td>
                                @can('work_tracking_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.work-trackings.show', $workTracking->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('work_tracking_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.work-trackings.edit', $workTracking->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('work_tracking_delete')
                                    <form action="{{ route('admin.work-trackings.destroy', $workTracking->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('work_tracking_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.work-trackings.massDestroy') }}",
