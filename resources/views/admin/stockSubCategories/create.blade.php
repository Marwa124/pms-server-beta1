...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.stockSubCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.stock-sub-categories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('stock_category') ? 'is-invalid' : '' }}" name="stock_category_id" id="stock_category_id" required>
                    @foreach($stock_categories as $id => $stock_category)
                        <option value="{{ $id }}" {{ old('stock_category_id') == $id ? 'selected' : '' }}>{{ $stock_category }}</option>
                    @endforeach
                </select>
