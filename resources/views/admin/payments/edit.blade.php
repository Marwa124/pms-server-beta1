    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('invoice') ? 'is-invalid' : '' }}" name="invoice_id" id="invoice_id" required>
                    @foreach($invoices as $id => $invoice)
                        <option value="{{ $id }}" {{ (old('invoice_id') ? old('invoice_id') : $payment->invoice->id ?? '') == $id ? 'selected' : '' }}>{{ $invoice }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paid_by') ? 'is-invalid' : '' }}" type="number" name="paid_by" id="paid_by" value="{{ old('paid_by', $payment->paid_by) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('transaction') ? 'is-invalid' : '' }}" name="transaction_id" id="transaction_id">
                    @foreach($transactions as $id => $transaction)
                        <option value="{{ $id }}" {{ (old('transaction_id') ? old('transaction_id') : $payment->transaction->id ?? '') == $id ? 'selected' : '' }}>{{ $transaction }}</option>
                    @endforeach
                </select>
