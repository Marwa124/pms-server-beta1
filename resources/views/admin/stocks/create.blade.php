    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.stocks.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('stock_sub_category') ? 'is-invalid' : '' }}" name="stock_sub_category_id" id="stock_sub_category_id" required>
                    @foreach($stock_sub_categories as $id => $stock_sub_category)
                        <option value="{{ $id }}" {{ old('stock_sub_category_id') == $id ? 'selected' : '' }}>{{ $stock_sub_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_stock') ? 'is-invalid' : '' }}" type="number" name="total_stock" id="total_stock" value="{{ old('total_stock', '') }}" step="1">
