...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.salaryPaymentDeduction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("payroll.admin.salary-payment-deductions.update", [$salaryPaymentDeduction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('salary_payment') ? 'is-invalid' : '' }}" name="salary_payment_id" id="salary_payment_id" required>
                    @foreach($salary_payments as $id => $salary_payment)
                        <option value="{{ $id }}" {{ (old('salary_payment_id') ? old('salary_payment_id') : $salaryPaymentDeduction->salary_payment->id ?? '') == $id ? 'selected' : '' }}>{{ $salary_payment }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $salaryPaymentDeduction->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" type="text" name="value" id="value" value="{{ old('value', $salaryPaymentDeduction->value) }}" required>
                @if($errors->has('value'))
                    <div class="invalid-feedback">
                        {{ $errors->first('value') }}
                    </div>
