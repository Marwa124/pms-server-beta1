...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.advanceSalary.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("payroll.admin.advance-salaries.update", [$advanceSalary->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $advanceSalary->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('advance_amount') ? 'is-invalid' : '' }}" type="text" name="advance_amount" id="advance_amount" value="{{ old('advance_amount', $advanceSalary->advance_amount) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('deduct_month') ? 'is-invalid' : '' }}" type="text" name="deduct_month" id="deduct_month" value="{{ old('deduct_month', $advanceSalary->deduct_month) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('reason') ? 'is-invalid' : '' }}" name="reason" id="reason">{!! old('reason', $advanceSalary->reason) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('request_date') ? 'is-invalid' : '' }}" type="text" name="request_date" id="request_date" value="{{ old('request_date', $advanceSalary->request_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="number" name="status" id="status" value="{{ old('status', $advanceSalary->status) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('approve_by') ? 'is-invalid' : '' }}" type="number" name="approve_by" id="approve_by" value="{{ old('approve_by', $advanceSalary->approve_by) }}" step="1">
