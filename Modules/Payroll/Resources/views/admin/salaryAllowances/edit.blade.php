...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("payroll.admin.salary-allowances.update", [$salaryAllowance->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.salaryAllowance.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $salaryAllowance->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" type="text" name="value" id="value" value="{{ old('value', $salaryAllowance->value) }}" required>
                @if($errors->has('value'))
                    <div class="invalid-feedback">
                        {{ $errors->first('value') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('salary_template') ? 'is-invalid' : '' }}" name="salary_template_id" id="salary_template_id" required>
                    @foreach($salary_templates as $id => $salary_template)
                        <option value="{{ $id }}" {{ (old('salary_template_id') ? old('salary_template_id') : $salaryAllowance->salary_template->id ?? '') == $id ? 'selected' : '' }}>{{ $salary_template }}</option>
                    @endforeach
                </select>
