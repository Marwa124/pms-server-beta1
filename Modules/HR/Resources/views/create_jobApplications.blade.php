...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("front.job-applications.store", $circularId->id) }}" enctype="multipart/form-data">
            @csrf
            {{-- <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job_circular') ? 'is-invalid' : '' }}" name="job_circular_id" id="job_circular_id" required>
                    @foreach($job_circulars as $id => $job_circular)
                        <option value="{{ $id }}" {{ old('job_circular_id') == $id ? 'selected' : '' }}>{{ $job_circular }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="mobile">{{ trans('cruds.jobApplication.fields.mobile') }}</label>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('cover_letter') ? 'is-invalid' : '' }}" name="cover_letter" id="cover_letter">{!! old('cover_letter') !!}</textarea>
            ...
            <div class="form-group">
                <label for="resume" class="required">{{ trans('cruds.jobApplication.fields.resume') }}</label>
            ...
            {{-- <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('application_status') ? 'is-invalid' : '' }}" name="application_status" id="application_status">
                    ...
                    @foreach($jobCircularModel::APPLICATION_STATUS_SELECT as $key => $label)
            ...
            {{-- <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('apply_date') ? 'is-invalid' : '' }}" type="text" name="apply_date" id="apply_date" value="{{ old('apply_date') }}">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '/front/job-applications/ckmedia', true);
...
<script>
    ...
    url: '{{ route('front.job-applications.storeMedia') }}',
    maxFilesize: 2, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
