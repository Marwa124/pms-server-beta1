...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("hr.admin.departments.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('department_head') ? 'is-invalid' : '' }}" name="department_head_id" id="department_head_id">
                    @foreach($department_heads as $id => $department_head)
                        <option value="{{ $id }}" {{ old('department_head_id') == $id ? 'selected' : '' }}>{{ $department_head }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mailbox') ? 'is-invalid' : '' }}" type="text" name="mailbox" id="mailbox" value="{{ old('mailbox', '') }}">
