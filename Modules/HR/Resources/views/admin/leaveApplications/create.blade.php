...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("hr.admin.leave-applications.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('leave_category') ? 'is-invalid' : '' }}" name="leave_category_id" id="leave_category_id" required>
                    @foreach($leave_categories as $id => $leave_category)
                        <option value="{{ $id }}" {{ old('leave_category_id') == $id ? 'selected' : '' }}>{{ $leave_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('leave_type') ? 'is-invalid' : '' }}" name="leave_type" id="leave_type" required>
                    ...
                    @foreach(Modules\HR\Entities\LeaveApplication::LEAVE_TYPE_SELECT as $key => $label)
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('leave_start_date') ? 'is-invalid' : '' }}" type="text" name="leave_start_date" id="leave_start_date" value="{{ old('leave_start_date') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('leave_end_date') ? 'is-invalid' : '' }}" type="text" name="leave_end_date" id="leave_end_date" value="{{ old('leave_end_date') }}">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '/admin/hr/leave-applications/ckmedia', true);
