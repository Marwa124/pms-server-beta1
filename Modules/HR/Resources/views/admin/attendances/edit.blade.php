...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.attendances.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.attendances.update", [$attendances->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $attendances->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('leave_application') ? 'is-invalid' : '' }}" name="leave_application_id" id="leave_application_id">
                    @foreach($leave_applications as $id => $leave_application)
                        <option value="{{ $id }}" {{ (old('leave_application_id') ? old('leave_application_id') : $attendances->leave_application->id ?? '') == $id ? 'selected' : '' }}>{{ $leave_application }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_in') ? 'is-invalid' : '' }}" type="text" name="date_in" id="date_in" value="{{ old('date_in', $attendances->date_in) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_out') ? 'is-invalid' : '' }}" type="text" name="date_out" id="date_out" value="{{ old('date_out', $attendances->date_out) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('attendance_status') ? 'is-invalid' : '' }}" name="attendance_status" id="attendance_status">
                    ...
                    @foreach(App\Models\attendances::ATTENDANCE_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('attendance_status', $attendances->attendance_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('clocking_status') ? 'is-invalid' : '' }}" type="number" name="clocking_status" id="clocking_status" value="{{ old('clocking_status', $attendances->clocking_status) }}" step="1">
