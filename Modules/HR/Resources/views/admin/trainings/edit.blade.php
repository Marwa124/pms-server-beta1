    ...
    <div class="card-body">
        <form method="POST" action="{{ route("hr.admin.trainings.update", [$training->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vendor_name') ? 'is-invalid' : '' }}" type="text" name="vendor_name" id="vendor_name" value="{{ old('vendor_name', $training->vendor_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('finish_date') ? 'is-invalid' : '' }}" type="text" name="finish_date" id="finish_date" value="{{ old('finish_date', $training->finish_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('training_cost') ? 'is-invalid' : '' }}" type="number" name="training_cost" id="training_cost" value="{{ old('training_cost', $training->training_cost) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" type="text" name="remarks" id="remarks" value="{{ old('remarks', $training->remarks) }}">
                @if($errors->has('remarks'))
                    <div class="invalid-feedback">
                        {{ $errors->first('remarks') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.training.fields.remarks_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('permissions') ? 'is-invalid' : '' }}" name="permissions[]" id="permissions" multiple>
                    @foreach($permissions as $id => $permission)
                        <option value="{{ $id }}" {{ (in_array($id, old('permissions', [])) || $training->permissions->contains($id)) ? 'selected' : '' }}>{{ $permission }}</option>
                    @endforeach
                </select>
